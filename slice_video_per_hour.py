        ...
        if filename.endswith((".mp4", ".mkv", ".avi")):
            ...
            if match:
                ...
                while True:
                    ...
                    output_filename = f"{filename.split('.')[0]}_{start_time_str}_{segment_index}.mp4"
                    output_filepath = os.path.join(output_directory, output_filename)

                    # Write 5-minute segment to a new video file
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    output_video = cv2.VideoWriter(output_filepath, fourcc, frame_rate, (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
